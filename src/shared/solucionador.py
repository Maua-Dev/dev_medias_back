        ...
        while(media_desejada + Solucionador.aumento_range <= Nota.DOMINIO_DE_NOTAS[-1]):
            ...
            for idx, nota in enumerate(
                ...
                valor_minimo = Utils.minimo_valor_no_dominio(notas_que_tenho=notas_que_tenho,
                                                            notas_que_quero=notas_que_quero[:idx] + \
                                                                            notas_que_quero[idx + 1:],
            ...
            if (len(notas_possiveis) != 0):
                melhor_combinacao = notas_possiveis[0]
                for nota in range(1, len(notas_possiveis)):
                    if(round(Utils.desvio_padrao(notas_possiveis[nota]), 4) == round(Utils.desvio_padrao(melhor_combinacao), 4)):
                        if(Utils.media(notas_que_tenho+list(notas_possiveis[nota])) < Utils.media(notas_que_tenho+list(melhor_combinacao))):
                            melhor_combinacao = notas_possiveis[nota]
                    elif (Utils.desvio_padrao(notas_possiveis[nota]) < Utils.desvio_padrao(melhor_combinacao)):
                        melhor_combinacao = notas_possiveis[nota]
                return list(melhor_combinacao)
